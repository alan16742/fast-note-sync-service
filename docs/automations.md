...
- `file_behavior`：匹配 Markdown 笔记、附件和目录的路径前缀、路径 Glob 以及创建、修改、删除、重命名等行为。路径相对于笔记库根目录：`*.md` 匹配根目录的 `a.md`，`notes/*.md` 匹配 `notes` 目录直接包含的 Markdown 文件；`*` 不跨越目录层级。前缀和 Glob 同时填写时必须同时满足，重命名时会检查旧路径与新路径。
...
一次笔记变更会分别参与“笔记内容”和“文件行为”条件匹配，但保留同一个事件 ID。同一条 OR 规则同时命中这两个分支时只执行一次；不同规则各自执行。文件行为规则只响应配置生效后的变更，不会因保存规则而扫描或补发已经存在的文件。
