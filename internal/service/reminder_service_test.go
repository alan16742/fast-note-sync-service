...
func TestReminderRetryPreservesSuccessfulTargetsAcrossRestart(t *testing.T) {
	env := newReminderTestEnv(t)
	ctx := context.Background()
	second := &reminderSender{err: errors.New("temporary failure")}
	target, err := env.webhooks.Save(ctx, &domain.WebhookSubscription{Provider: "serverchan", Secret: "test"}, 1)
	require.NoError(t, err)
	webhookSvc := env.svc.webhooks.(*webhookService)
	webhookSvc.senders["serverchan"] = second
	env.trigger.Actions = append(env.trigger.Actions, domain.AutomationAction{Type: domain.AutomationTargetWebhook, ConfigID: target.ID})
	_, err = env.triggers.Save(ctx, env.trigger, 1)
	require.NoError(t, err)
	require.NoError(t, env.svc.Tick(ctx, env.start))
	due := env.start.Add(5 * time.Minute)
	...
	second.err = nil
	restarted := NewReminderService(reminderUsers{}, env.vaults, env.source, env.triggers, webhookSvc, env.jobs, zap.NewNop())
	require.NoError(t, restarted.Tick(ctx, due.Add(time.Minute)))
	require.Len(t, env.sender.calls, 1, "successful channel must not receive a duplicate")
	require.Len(t, second.calls, 2)
	require.NoError(t, restarted.Tick(ctx, due.Add(time.Hour)))
	require.Len(t, env.sender.calls, 2, "the next scheduled offset must notify every channel again")
	require.Len(t, second.calls, 3)
}

func TestReminderCheckpointRenewsLeaseAndClaimReadsLatestProgress(t *testing.T) {
	env := newReminderTestEnv(t)
	ctx := context.Background()
	require.NoError(t, env.svc.Tick(ctx, env.start))
	now := env.start.Add(5 * time.Minute).Unix()
	...
	require.Len(t, rows, 1)
	id := rows[0].ID
	claimed, err := env.jobs.Claim(ctx, 1, id, now, "first")
	require.NoError(t, err)
	require.NotNil(t, claimed)
	require.NoError(t, env.jobs.Checkpoint(ctx, 1, id, now+59, "first", []int64{7}))
	claimed, err = env.jobs.Claim(ctx, 1, id, now+61, "second")
	require.NoError(t, err)
	require.Nil(t, claimed, "renewal must protect a long multi-channel delivery")
	require.NoError(t, env.jobs.Retry(ctx, 1, id, "first", now+90))
	claimed, err = env.jobs.Claim(ctx, 1, id, now+90, "second")
	require.NoError(t, err)
	require.Equal(t, []int64{7}, claimed.DeliveredTargets, "claim must not reuse the earlier empty ListDue snapshot")
	require.ErrorContains(t, env.jobs.Checkpoint(ctx, 1, id, now+90, "first", nil), "lease was lost")
}
